...
from data.sugarbeet_dataloader import SugarBeet_Dataset
...
parser.add_argument("--root_dir", type=str, default='C:/Users/ADMIN/MySGNet/data/sugarbeet', 
                    help="root dir of SugarBeet dataset")
parser.add_argument("--batchsize", type=int, default=4, help="batchsize of training dataloader")  # TĂNG LÊN 4
parser.add_argument("--max_samples", type=int, default=1000, help="max samples for training")
parser.add_argument("--pretrained", type=str, 
                    default='C:/Users/ADMIN/MySGNet/experiments_vedai/VEDAI-finetune-from-kaist-lr_5e-05-s_4-b_4-20260216105955/best_model_rmse_0.0086_epoch98.pth',
                    help="path to pretrained VEDAI model")
...
if __name__ == '__main__':
    ...
    result_root = '%s/SugarBeet-finetune-from-vedai-lr_%s-s_%s-b_%s-%s' % (opt.result, opt.lr, opt.scale, opt.batchsize, s)
    ...
    # Load pretrained weights từ VEDAI
    print(f"\nLoading pretrained VEDAI model from: {opt.pretrained}")
    print("  → Model đã được train trên FLIR → KAIST → VEDAI")
    checkpoint = torch.load(opt.pretrained)
    net.load_state_dict(checkpoint)
    print("✓ Pretrained weights loaded successfully!")
    print(f"  → Best RMSE from VEDAI: 0.0086")
    ...
    # Load SugarBeet dataset
    print("\nLoading SugarBeet dataset for fine-tuning...")
    train_dataset = SugarBeet_Dataset(
    ...
    print(f"\n=== Starting SugarBeet Fine-tuning from VEDAI ===")
    print(f"Train batches: {num_train}")
    print(f"Val batches: {len(val_dataloader)}")
    print(f"Total epochs: {max_epoch}")
    print(f"Training history: FLIR → KAIST → VEDAI → SugarBeet")
    print(f"Patch size: 128x160 (GT), 32x40 (LR)")  # THÊM DÒNG NÀY
        ...
        for idx, data in enumerate(t):
            try:
                ...
                if idx == 0 and epoch == 0:
                    print(f"\nGuidance shape: {guidance.shape}")  # [3, 128, 160]
                    print(f"LR shape: {lr.shape}")                # [1, 32, 40]
                    print(f"GT shape: {gt.shape}")                # [1, 128, 160]
                    print(f"Patch size: 128x160 → OK!\n")
                ...
                if idx % 50 == 0:
                    t.set_description('[SugarBeet finetune epoch:%d] loss: %.6f' % (epoch + 1, loss.data.item()))
        ...
        if epoch % 2 == 0:
            with torch.no_grad():
                ...
                for idx, data in enumerate(t):
                    ...
                    t.set_description('[SugarBeet validate] rmse: %.4f' % rmse[:idx + 1].mean())
